...
const unsigned int &ProgressBar::increase_performed(const unsigned int delta_steps)
{
...
const unsigned int &ProgressBar::increase_performed_to(const unsigned int performed)
{  
  std::unique_lock<std::mutex> lock(_mutex);
...
const unsigned int &ProgressBar::unsafe_increase_performed_to(const unsigned int performed)
{
  ...
  if (_num_of_bar_dots > 0 && _expected > 0) {
      ...
      const unsigned int already_displayed = int(dot_per_step*_represented_steps);
      const unsigned int to_display = int(dot_per_step*_performed);

      for (unsigned int dot = already_displayed+1; dot<=to_display; ++dot) {
        _bstream << "#" << std::flush;
        if (_num_of_bar_dots>10 && int((dot-1)*5.0/_num_of_bar_dots)<int(dot*5.0/_num_of_bar_dots)) {
          _bstream << " " << int(dot*100.0/_num_of_bar_dots) << "% " << std::flush;
        }
      }
  }
