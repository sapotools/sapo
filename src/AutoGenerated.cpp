...
Bundle *getBundleWithAssumptions(const InputData &id, Context ctx);
...
AutoGenerated::AutoGenerated(const InputData &id, Context ctx): Model()
{
  using namespace std;
  ...
  for (unsigned i = 0; i < id.getVarNum(); i++) {
		SymbolicAlgebra::Expression<> dyn = id.getVar(i)->getDynamic();
		dyn = expandDefinitions(dyn, ctx);
    this->dyns.push_back(dyn);
  }


	this->reachSet = getBundleWithAssumptions(id, ctx);
  ...
  if (id.paramDirectionsNum() != 0) {
    ...
    for (unsigned i = 0; i < id.paramDirectionsNum(); i++) {
      pA[2 * i] = id.getParamDirection(i)->getDirectionVector(ctx, false);
    ...
    for (unsigned i = 0; i < id.paramDirectionsNum(); i++) {
      pb[2 * i] = id.getParamDirection(i)->getUB(ctx);
      pb[2 * i + 1] = -id.getParamDirection(i)->getLB(ctx);
    }
  ...
  // formula
  if (id.isSpecDefined()) {
    this->spec = id.getSpec()->toSTL(ctx, vars, params);
  } else {
    this->spec = NULL;
  }
}
...
Bundle *getBundleWithAssumptions(const InputData &id, Context ctx)
{
	...
	for (unsigned i = 0; i < id.getDirectionsNum(); i++) {
		directions.push_back(id.getDirection(i)->getDirectionVector(ctx, true));
		UB.push_back(id.getDirection(i)->getUB(ctx));
		LB.push_back(id.getDirection(i)->getLB(ctx));
	}
	...
	for (unsigned i = 0; i < id.getAssumptionsNumber(); i++) {
		...
		std::vector<double> new_dir = id.getAssumption(i)->getDirectionVector(ctx, true);
		...
		if (pos_dir != -1) {			// constrain a direction which is in the L matrix
			...
			constrOffsets.push_back(rescale(id.getAssumption(i)->getOffset(ctx), new_dir, constrDirs[i]));
			...
			if (C_pos != -1) {			// direction is already constrained, change offset and UB
				...
				constrOffsets[directions.size() + C_pos] = std::min(
					constrOffsets[directions.size() + C_pos], id.getAssumption(i)->getOffset(ctx));
				...
				constrOffsets.push_back(id.getAssumption(i)->getOffset(ctx));
				LB[directions.size() + C_negated_pos] = -id.getAssumption(i)->getOffset(ctx);
...
//			if (glp_mip_col_val(lp, map_paral(d, P, Pn) + 1) == 1) {
				T[P].push_back(d);
			}
		}
	}
