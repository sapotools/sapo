...
AutoGenerated::AutoGenerated(const InputData &id): Model()
{
  ...
  if (id.isDynamicCompositionEnabled() && id.getDynamicDegree() > 1) {
		...
		for (unsigned v = 0; v < vars.size(); v++) {
			std::string name = SymbolicAlgebra::Symbol<>::get_symbol_name(vars[v].get_id());
			name += "_2";
		...
		for (unsigned i = 1; i < id.getDynamicDegree(); i++) {
			for (unsigned v = 0; v < id.getVarNum(); v++) {
				...
				for (unsigned var = 0; var < vars.size(); var++) {
					rep_symb[(SymbolicAlgebra::Symbol<>) vars[var]] = new_var_symbols[var];
				}
  ...
  if (id.paramDirectionsNum() != 0) {
    ...
    for (unsigned i = 0; i < id.paramDirectionsNum(); i++) {
      pA[2 * i] = id.getParamDirection(i)->getDirectionVector(id.getParamSymbols());
...
Bundle *getBundleWithAssumptions(const InputData &id)
{
	...
	for (unsigned i = 0; i < id.getDirectionsNum(); i++) {
		directions.push_back(id.getDirection(i)->getDirectionVector(id.getVarSymbols()));
	...
	for (unsigned i = 0; i < id.getAssumptionsNumber(); i++) {
		...
		std::vector<double> new_dir = id.getAssumption(i)->getDirectionVector(id.getVarSymbols());
		...
		if (pos_dir != -1) {			// constrain a direction which is in the L matrix
			...
			constrOffsets.push_back(rescale(id.getAssumption(i)->getOffset(), new_dir, constrDirs[i]));
			...
			if (C_pos != -1) {			// direction is already constrained, change offset and UB
				...
				constrOffsets[directions.size() + C_pos] = std::min(
					constrOffsets[directions.size() + C_pos], id.getAssumption(i)->getOffset());
				...
				LB[directions.size() + C_negated_pos] = -id.getAssumption(i)->getOffset();
