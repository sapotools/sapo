...
AutoGenerated::AutoGenerated(const InputData &id): Model()
{
  ...
  if (id.paramDirectionsNum() != 0) {
    ...
    for (unsigned i = 0; i < id.paramDirectionsNum(); i++) {
      pA[2 * i]
          = id.getParamDirection(i)->getDirectionVector(id.getParamSymbols());
...
Bundle *getBundleWithAssumptions(const InputData &id)
{
  ...
  for (unsigned i = 0; i < id.getDirectionsNum(); i++) {
    directions.push_back(
        id.getDirection(i)->getDirectionVector(id.getVarSymbols()));
  ...
  for (unsigned i = 0; i < id.getAssumptionsNumber(); i++) {
    ...
    std::vector<double> new_dir
        = id.getAssumption(i)->getDirectionVector(id.getVarSymbols());
 ...
 */
bool on_the_same_line(const std::vector<double> &v1, const std::vector<double> &v2)
{
  if (v1.size() != v2.size()) {
 ...
 * @return the index in `M` of a direction which lays on the same line as `v` if it exists; 
 *         an invalid index for `M`, otherwise
 */

size_t find_on_the_same_line(const std::vector<std::vector<double>> &M, const std::vector<double> &v)
{
  ...
  for (unsigned P = 0; P < Pn; P++) {
    for (unsigned d = 0; d < m + c; d++) {
      if (glp_get_col_prim(lp, map_paral(d, P, Pn) + 1) == 1) {
        //			if (glp_mip_col_val(lp, map_paral(d, P, Pn) + 1) ==
        //1) {
        T[P].push_back(d);
      }
    }
