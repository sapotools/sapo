...
// creates the bundle defined by the model, adding directions and template rows according to assumptions
...
void directionUseConstraints(glp_prob **lp, const std::vector<std::vector<double>> A,
													const std::vector<std::vector<double>> C, unsigned *startingIndex, unsigned Pn);

// adds to lp the constraints that fix directions in parallelotopes according to old template
void oldTemplateConstraints(glp_prob **lp, const std::vector<std::vector<double>> A,
													const std::vector<std::vector<double>> C, unsigned *startingIndex,
													std::vector<std::vector<int>> oldTemplate, unsigned Pn);
...
double findDirectionBound(std::vector<std::vector<double>> A, std::vector<double> LB,
													std::vector<double> UB, std::vector<double> dir, bool minimize);
 ...
 *  - we require that each variable v of the dynamical system is covered in each parallelotope
 *  - we require that each direction d appears in at least one parallelotope.
 * 
 * We also require that the user-specified template is included in the solution.
 * 
...
AutoGenerated::AutoGenerated(const InputData &m)
{
  ...
  for (unsigned i = 0; i < m.getVarNum(); i++) {
	...
	if (m.getAssumptionsNumber() != 0) {
		this->reachSet = getBundleWithAssumptions(m);
	} else {
		this->reachSet = new Bundle(m.getDirections(), m.getUB(), -m.getLB(), m.getTemplate());
	}
...
//				double min_val = LS.minLinearSystem(symbols, obj_function);
	...
	std::vector<bool> isVarCovered(id.getVarNum(), false);
	std::vector<bool> isDirectionUsed(id.directionsNum(), false);
	if (id.templateRows() > 0) {
		for (unsigned i = 0; i < id.templateRows(); i++) {
			for (unsigned j = 0; j < id.getTemplate()[i].size(); j++) {
				int dir = id.getTemplate()[i][j];
				isDirectionUsed[dir] = true;
				for (unsigned k = 0; k < directions[0].size(); k++) {
					if (directions[dir][k] != 0) {
						isVarCovered[k] = true;
					}
				}
			}
...
{
	// fix value of directions that are in the old template
	int fixedVars = 0;
	if (oldTemplate.size() != 0) {
		fixedVars = oldTemplate.size() * oldTemplate[0].size();
	}
	
	std::cout << "old template = " << oldTemplate << std::endl;
	...
	unsigned rows = Pn + Pn*n + (m + c) + fixedVars;

	std::cout << "uncovered rows = " << uncoveredRows << std::endl;
	std::cout << "c = " << c << std::endl;
	std::cout << "n = " << n << std::endl;
	
	std::cout << "old template size = " << oldTemplate.size() << std::endl;
...
void directionUseConstraints(glp_prob **lp,
							std::vector<std::vector<double>> A, std::vector<std::vector<double>> C, unsigned *startingIndex, unsigned Pn)
{
	//unsigned n = A[0].size();		// var num, = cols of A
	unsigned m = A.size();				// dirs num = rows of A
	unsigned c = C.size();				// constr num = assumptions = rows of C
...
//	unsigned Pn = ceil(((double) (m + c))/n);		// number of parallelotopes required
	...
	for (unsigned P = 0; P < oldTemplate.size(); P++) {
		for (unsigned d = 0; d < oldTemplate[P].size(); d++) {
			std::cout << map(d, P, Pn) << "is constrained to be 1" << std::endl;
			int *indeces = (int *) malloc((cols + 1) * sizeof(int));
			double *constr = (double *) malloc((cols + 1) * sizeof(double));
			int len = 1;
			indeces[0] = 0;
...
double findDirectionBound(std::vector<std::vector<double>> A, std::vector<double> LB, std::vector<double> UB, std::vector<double> dir, bool minimize)
{
