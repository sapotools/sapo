...
AutoGenerated::AutoGenerated(const InputData &m): Model()
{
  ...
  if (m.paramDirectionsNum() != 0) {
    ...
    for (unsigned i = 0; i < m.paramDirectionsNum(); i++) {
      pA[2 * i] = m.getParamDirection(i)->getDirectionVector(m, false);
    ...
    for (unsigned i = 0; i < m.paramDirectionsNum(); i++) {
      pb[2 * i] = m.getParamDirection(i)->getUB(m);
      pb[2 * i + 1] = -m.getParamDirection(i)->getLB(m);
    }
...
Bundle *getBundleWithAssumptions(const InputData &id)
{
	...
	for (unsigned i = 0; i < id.getDirectionsNum(); i++) {
		directions.push_back(id.getDirection(i)->getDirectionVector(id, true));
		UB.push_back(id.getDirection(i)->getUB(id));
		LB.push_back(id.getDirection(i)->getLB(id));
	}
	...
	for (unsigned i = 0; i < id.getAssumptionsNumber(); i++) {
		...
		std::vector<double> new_dir = id.getAssumption(i)->getDirectionVector(id, true);
