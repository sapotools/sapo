...
AutoGenerated::AutoGenerated(const InputData &m): Model()
{
  ...
  if (m.getAssumptionsNumber() != 0) {
    ...
    this->reachSet = new Bundle(m.getDirections(), m.getUB(), -m.getLB(),
                                m.getTemplate());
  }
  ...
  for (unsigned i = 0; i < id.getAssumptionsNumber(); i++) {
    ...
    if (pos_dir != -1) { // constrain a direction which is in the L matrix
      ...
      constrOffsets.push_back(
          rescale(id.getAssumption(i)->getOffset(id), new_dir, constrDirs[i]));
      ...
      if (C_pos
        ...
        constrOffsets[directions.size() + C_pos]
            = std::min(constrOffsets[directions.size() + C_pos],
                       id.getAssumption(i)->getOffset(id));
        ...
        //				double min_val = LS.minLinearSystem(symbols,
        //obj_function);
        double min_val = findDirectionBound(id.getDirections(), id.getLB(),
                                            id.getUB(), new_dir, true);
                ...
                std::vector<std::vector<int>> oldTemplate)
{
  ...
  // unsigned Pn = ceil(((double) (m + c))/n);		// number of parallelotopes
  // required
  ...
  /*	std::cout << "uncovered rows = " << uncoveredRows << std::endl;
          ...
          std::cout << "old template size = " << oldTemplate.size() <<
     std::endl;
  */
  ...
  for (unsigned P = 0; P < Pn; P++) {
    for (unsigned d = 0; d < m + c; d++) {
      //			std::cout << "X_" << d << "_" << P << " = " <<
      //glp_get_col_prim(lp, map(d, P, Pn) + 1) << std::endl;
                          ...
                          unsigned *startingIndex, unsigned Pn)
{
  ...
  unsigned m = A.size();    // dirs num = rows of A
  unsigned c = C.size();    // constr num = assumptions = rows of C
  //	unsigned Pn = ceil(((double) (m + c))/n);		// number of
  //parallelotopes required
                            ...
                            unsigned Pn)
{
  ...
  for (unsigned P = 0; P < oldTemplate.size(); P++) {
    for (unsigned d = 0; d < oldTemplate[P].size(); d++) {
      //			std::cout << map(d, P, Pn) << "is constrained
      //to be 1" << std::endl;
      int *indeces = (int *)malloc((cols + 1) * sizeof(int));
      double *constr = (double *)malloc((cols + 1) * sizeof(double));
      int len = 1;
