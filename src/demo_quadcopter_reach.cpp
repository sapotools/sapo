/**
 * @file demo_quadcopter.cpp
 * Demo: Reachability analysis of 17 dimensional quadcopter model taken from
...
int main(int argc,char** argv){
	...
	Model *quadcopter = new Model(vars,dyns);
	...
	Sapo *sapo = new Sapo(quadcopter,options);
		int reach_steps = 300;
		Flowpipe* flowpipe = sapo->reach(B,reach_steps);	// reachability analysis

		// Store the constructed flowpipes in files lotka_volterra_proj_x*.m (in Matlab format)
		char file_name_1[] = "hb.m";
		char file_name_2[] = "wb.m";
		char file_name_3[] = "hib.m";

		flowpipe->plotProjToFile(2,0.01,file_name_1,'w');
		flowpipe->plotProjToFile(5,0.01,file_name_2,'w');
		flowpipe->plotProjToFile(13,0.01,file_name_3,'w');
