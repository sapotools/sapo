 ...
 */
Negation::Negation(const std::shared_ptr<STL> f): STL(NEGATION), f(f) {}
...
const std::shared_ptr<STL> Negation::simplify() const
{
  ...
  case formula_type::CONJUNCTION: {
    std::shared_ptr<Conjunction> conj
        = std::dynamic_pointer_cast<Conjunction>(f);
    std::shared_ptr<STL> left
        = std::make_shared<Negation>(conj->getLeftSubFormula())->simplify();
    std::shared_ptr<STL> right
        = std::make_shared<Negation>(conj->getRightSubFormula())->simplify();
    return std::make_shared<Disjunction>(left, right);
  }
  case formula_type::DISJUNCTION: {
    std::shared_ptr<Disjunction> disj
        = std::dynamic_pointer_cast<Disjunction>(f);
    std::shared_ptr<STL> left
        = std::make_shared<Negation>(disj->getLeftSubFormula())->simplify();
    std::shared_ptr<STL> right
        = std::make_shared<Negation>(disj->getRightSubFormula())->simplify();
  ...
  case formula_type::ALWAYS: {
    ...
    std::shared_ptr<STL> sub
        = std::make_shared<Negation>(alw->getSubFormula())->simplify();
  ...
  case formula_type::EVENTUALLY: {
    std::shared_ptr<Eventually> event
        = std::dynamic_pointer_cast<Eventually>(f);
    std::shared_ptr<STL> sub
        = std::make_shared<Negation>(event->getSubFormula())->simplify();
