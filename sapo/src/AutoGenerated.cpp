...
LinearSystem getAssumptions(const InputData &id)
{
  using namespace LinearAlgebra;
  ...
  for (unsigned i = 0; i < id.getAssumptionsNumber(); i++) {
    const Direction *direction = id.getAssumption(i);

    switch (direction->getType()) {
      case Direction::LE:
      case Direction::GE:
        constrDirs.push_back(direction->getConstraintVector(variables));
        constrOffsets.push_back(direction->getOffset());
        break;
      case Direction::EQ:
        constrDirs.push_back(direction->getConstraintVector(variables));
        constrOffsets.push_back(direction->getOffset());
      ...
      case Direction::IN:
        constrDirs.push_back(direction->getConstraintVector(variables));
        constrOffsets.push_back(direction->getUB());
        constrDirs.push_back(-direction->getConstraintVector(variables));
        constrOffsets.push_back(-direction->getLB());
        break;
      default:
        throw std::domain_error("Unsupported relation in assumption");
    }
  }
...
Model get_model(const InputData &id)
{
  ...
  // variables
  std::vector<Symbol<>> variables(id.getVarNum());
  for (unsigned i = 0; i < id.getVarNum(); i++) {
    variables[i] = id.getVar(i)->getSymbol();
  }

  // parameters
  std::vector<Symbol<>> parameters(id.getParamNum());
  for (unsigned i = 0; i < id.getParamNum(); i++) {
    parameters[i] = id.getParam(i)->getSymbol();
  }
  ...
  model.set_assumptions(getAssumptions(id));
