...
private:
  ...
  bool validTemp(vector<vector<int>> T, unsigned int card,
                 vector<int> dirs); // check if a template is valid
...
public:
  ...
  const vector<int> &getTemplate(long unsigned int i) const
  {
    return this->T[i];
  }
  ...
  Bundle transform(const lst &vars, const lst &f, CtrlPointType &controlPts,
                   int mode) const;
  Bundle transform(const lst &vars, const lst &params, const lst &f,
                   const LinearSystem &paraSet, CtrlPointType &controlPts,
                   int mode) const;
