...
BOOST_AUTO_TEST_CASE(test_bundle)
{
    ...
    BOOST_CHECK(is_true(b1==Polytope(Ap, {5,5,5,3,7,0,0,0,0,0})));
    BOOST_CHECK(is_true(b1==b2));
    BOOST_CHECK(is_true(b1==b3));
    BOOST_CHECK(is_true(b1!=b4));
    BOOST_CHECK(is_true(b1!=b5));
    BOOST_CHECK(is_true(b1!=b6));
    BOOST_CHECK(is_true(b4==b5));
    BOOST_CHECK(is_true(b1!=Polytope(Ap, {5,5,5,10,7,0,0,0,0,0})));
...
BOOST_AUTO_TEST_CASE(test_is_empty_bundle)
{
    ...
    BOOST_CHECK(is_true(b2.is_empty()));
}
...
BOOST_AUTO_TEST_CASE(test_are_disjoint_polytope)
{
    ...
    BOOST_CHECK(is_false(are_disjoint(b1, b1)));
    BOOST_CHECK(is_false(are_disjoint(b1, b2)));
    BOOST_CHECK(is_false(are_disjoint(b1, b3)));
    ...
    BOOST_CHECK(is_false(are_disjoint(b2, b1)));
    BOOST_CHECK(is_false(are_disjoint(b2, b2)));
    ...
    BOOST_CHECK(is_false(are_disjoint(b3, b1)));
    BOOST_CHECK(is_false(are_disjoint(b3, b2)));
    BOOST_CHECK(is_false(are_disjoint(b3, b3)));
    BOOST_CHECK(is_false(are_disjoint(b3, b4)));
    BOOST_CHECK(is_true(are_disjoint(b3, b5)));
    BOOST_REQUIRE_THROW(are_disjoint(b3, b6), std::domain_error);

    BOOST_CHECK(is_true(are_disjoint(b4, b1)));
    BOOST_CHECK(is_true(are_disjoint(b4, b2)));
    BOOST_CHECK(is_false(are_disjoint(b4, b3)));
    BOOST_CHECK(is_false(are_disjoint(b4, b4)));
    BOOST_CHECK(is_true(are_disjoint(b4, b5)));
    BOOST_REQUIRE_THROW(are_disjoint(b4, b6), std::domain_error);

    BOOST_CHECK(is_true(are_disjoint(b5, b1)));
    BOOST_CHECK(is_true(are_disjoint(b5, b2)));
    BOOST_CHECK(is_true(are_disjoint(b5, b3)));
    BOOST_CHECK(is_true(are_disjoint(b5, b4)));
    BOOST_CHECK(is_true(are_disjoint(b5, b5)));
    ...
    BOOST_REQUIRE(is_false(are_disjoint(b6, b6)));
}
...
BOOST_AUTO_TEST_CASE(test_intersect_bundle)
{
    ...
    ba = intersect(b1,b2);
    BOOST_CHECK(is_true(ba==b1));
    BOOST_CHECK(is_true(ba==b2));
    BOOST_CHECK(is_true(ba==intersect(b2,b1)));

    ba = intersect(b1,b3);
    BOOST_CHECK(is_true(ba==b1));
    BOOST_CHECK(is_true(ba!=b3));
    BOOST_CHECK(is_true(ba==intersect(b3,b1)));

    ba = intersect(b1,b4);
    BOOST_CHECK(is_true(ba==b4));
    BOOST_CHECK(is_true(ba!=b1));
    BOOST_CHECK(is_true(ba==intersect(b4,b1)));

    ba = intersect(b1,b5);
    BOOST_CHECK(is_true(ba.is_empty()));
    BOOST_CHECK(is_true(b5.is_empty()));
    BOOST_CHECK(is_false(b1.is_empty()));
...
BOOST_AUTO_TEST_CASE(test_is_subset_of_bundle)
{
    ...
    BOOST_CHECK(is_true(b1.is_subset_of(b2)));
    BOOST_CHECK(is_false(b2.is_subset_of(b1)));

    BOOST_CHECK(is_true(b1.is_subset_of(b3)));
    BOOST_CHECK(is_false(b3.is_subset_of(b1)));

    BOOST_CHECK(is_true(b4.is_subset_of(b1)));
    BOOST_CHECK(is_false(b1.is_subset_of(b4)));

    BOOST_CHECK(is_false(b1.is_subset_of(b5)));
    BOOST_CHECK(is_true(b5.is_subset_of(b1)));
    ...
    for (const Bundle& bundle: {b1, b2, b3, b4, b5, b6}) {
        ...
        if (!is_true(bundle.is_empty())) {
            BOOST_CHECK(is_false(bundle.is_subset_of(b6)));
        }
    }
}
...
BOOST_AUTO_TEST_CASE(test_includes_bundle)
{
    ...
    BOOST_CHECK(is_true(b2.includes(b1)));
    BOOST_CHECK(is_false(b1.includes(b2)));

    BOOST_CHECK(is_true(b3.includes(b1)));
    BOOST_CHECK(is_false(b1.includes(b3)));

    BOOST_CHECK(is_true(b1.includes(b4)));
    BOOST_CHECK(is_false(b4.includes(b1)));

    BOOST_CHECK(is_false(b5.includes(b1)));
    BOOST_CHECK(is_true(b1.includes(b5)));
    ...
    for (const Bundle& bundle: {b1, b2, b3, b4, b5, b6}) {
        ...
        if (!is_true(bundle.is_empty())) {
            BOOST_CHECK(is_false(b6.includes(bundle)));
        }
    }
}
...
BOOST_AUTO_TEST_CASE(test_intersect_with_ls_bundle)
{
    ...
    BOOST_CHECK(is_true(ba==Polytope(C, {5,5,5,3,7,5,7,0,0,0,0,0,0,0})));

    ba = b;
    ba.intersect_with(ls2);
    BOOST_CHECK(is_true(b.includes(ba)));
    BOOST_CHECK(is_true(ba.satisfies(ls2)));
    BOOST_CHECK(is_true(ba==b));

    ba = b;
    ba.intersect_with(ls3);
    BOOST_CHECK(is_true(b.includes(ba)));
    BOOST_CHECK(is_true(ba.satisfies(ls3)));
    BOOST_CHECK(is_true(ba==Polytope(C, {5,5,5,3,7,100,7,0,0,0,0,0,0,0})));

    ba = b;
    ba.intersect_with(ls4);
    BOOST_CHECK(is_true(b.includes(ba)));
    BOOST_CHECK(is_true(ba.satisfies(ls4)));
    BOOST_CHECK(is_true(ba==Polytope(C, {5,5,5,3,7,5,7,0,0,0,0,0,0,100})));
    ...
    BOOST_CHECK(is_true(b.includes(ba)));
    BOOST_CHECK(is_true(ba.satisfies(ls5)));
    BOOST_CHECK(is_true(ba==empty_poly));

    ba = b;
    ba.intersect_with(ls6);
    BOOST_CHECK(is_true(b.includes(ba)));
    BOOST_CHECK(is_true(ba.satisfies(ls6)));
    BOOST_CHECK(is_true(ba==empty_poly));

    ba = b;
    ba.intersect_with(ls7);
    BOOST_CHECK(is_true(b.includes(ba)));
    BOOST_CHECK(is_true(ba.satisfies(ls7)));
    BOOST_CHECK(is_true(ba==empty_poly));
}
