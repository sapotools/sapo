...
{
private:
  const std::shared_ptr<STL> _subformula; //!< eventually sub-formula
...
public:
  ...
  /**
   * @brief Get the eventually sub-formula
   * 
   * @return the eventually sub-formula
   */
  ...
  {
    return std::make_shared<Eventually>(_t_interval.begin(), _t_interval.end(),
                                        _subformula->get_PNF());
  }
