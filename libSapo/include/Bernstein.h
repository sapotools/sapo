 ...
 */
static 
std::vector<unsigned int> get_shifts(const std::vector<unsigned int> &degrees)
{
  std::vector<unsigned int> shifts(degrees.size(), 0);

  shifts[degrees.size() - 1]
 ...
 * @param[in] polynomial is the polynomial whose coefficients must be evaluated 
 ...
 */
static
void get_coeffs_aux(std::vector<SymbolicAlgebra::Expression<C>> &coeffs,
                    ...
                    unsigned int idx, const unsigned int position)
{
  if (polynomial == 0) {
    return;
  }
 ...
 */
static
std::vector<SymbolicAlgebra::Expression<C>> 
 ...
 */
static
std::vector<unsigned int> get_degree(const SymbolicAlgebra::Expression<C> &p,
 ...
 */
static
std::vector<unsigned int> get_degree(const SymbolicAlgebra::Expression<C> &polynomial,
                                     const std::vector<SymbolicAlgebra::Symbol<C>> &vars)
{
  std::vector<unsigned int> degrees;
 ...
 */
static
std::vector<std::vector<SymbolicAlgebra::Expression<C>>>
...
//! @private
static
std::vector<unsigned int> increase_by_one(const std::vector<unsigned int>& orig)
{
 ...
 */
static
LinearAlgebra::Dense::Matrix<SymbolicAlgebra::Expression<C>>
...
static
std::vector<SymbolicAlgebra::Expression<C>>
get_coefficients_from_polynomial(const std::vector<SymbolicAlgebra::Symbol<C>> &vars,
                                const SymbolicAlgebra::Expression<C> &polynomial,
                                const std::vector<unsigned int> &degrees,
                                const std::vector<unsigned int> &shifts)
{
  ...
  auto UAt
      = transp(genUtilde(degrees[0]) * A, degrees_p);
                                 ...
                                 const SymbolicAlgebra::Expression<C> &polynomial)
{
  if (!polynomial.is_a_polynomial()) {
    throw std::domain_error("get_coefficients_from_polynomial: the parameter is not "
                            " a polynomial");
  }
...
inline
std::vector<SymbolicAlgebra::Expression<C>>
get_Bernstein_coefficients_from_polynomial(const std::vector<SymbolicAlgebra::Symbol<C>> &vars,
                                           const SymbolicAlgebra::Expression<C> &polynomial)
{
